...
async function populateLeaderboard() {
  ...
  let current_username = await getLogin();
  ...
  for (let cat of categories) {
    ...
    for (let user of leaderboard[cat]) {
      ...
      // If the current category is time_played then our format will change to to hour-min-sec time
      if (cat ==  categories[3]) {
      ...
      await createLeaderboardEntry(user['login'], current_username, value, cat);
    }
  }
...
async function sortCategory(category, isAscend) {
  ...
  let results = await fetch('../PHP/getLBCat.php', {
    method: 'POST',
    body: formData
  })
  .then((response) => {
  ...
  for (let result of results) {
    createLeaderboardEntry(result['login'], await getLogin(), result['value'], category);
  }
}
